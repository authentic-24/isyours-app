...
<main>
    <div class="ln-center">
        ...
        {{-- Branding --}}
        <div class="ln-brand">
            <img src="{{ asset('images/logo-main.svg') }}" alt="Is Yours" class="ln-brand-logo">
            <p>{{ __('home.login_subtitle') }}</p>
        </div>
        ...
        <div class="ln-card">
            ...
            <form class="ln-form" method="POST" action="{{ route('login_post') }}" novalidate autocomplete="off">
                @csrf

                {{-- Email --}}
                <div>
                    <label class="ln-label" for="email">{{ __('home.email_address') }}</label>
                ...
                <div>
                    <div class="ln-field-label-row">
                        <label class="ln-label" for="password">{{ __('home.password') }}</label>
                        <a class="ln-forgot" href="#">{{ __('home.forgot_password') }}</a>
                    </div>
                ...
                {{-- Submit --}}
                <button type="submit" class="ln-submit">{{ __('home.sign_in') }}</button>
            </form>

            {{-- Divider --}}
            <div class="ln-divider">
                <div class="ln-divider-line"><div></div></div>
                <div class="ln-divider-label"><span>{{ __('home.or_continue_with') }}</span></div>
            </div>

            {{-- Register --}}
            <p class="ln-register">
                {{ __('home.dont_have_account') }}
                <a href="{{ route('web_register') }}">{{ __('home.create_account') }}</a>
            </p>
...
<div class="lang-fab" id="langFab">
    <button class="lang-fab-btn" id="langFabBtn" aria-label="Change language">
        ...
        <span style="font-size:13px;font-weight:700;letter-spacing:.05em">{{ strtoupper(app()->getLocale()) }}</span>
    </button>
