...
@section('dashboard_title', 'My Skills')
...
<div class="sk-wrap">
    ...
    <form id="sk-form" action="{{ route('web.candidate.update') }}" method="post">
        @csrf
        ...
        <div class="sk-panel active" id="sk-panel-1">
            <div class="sk-card">
                <div class="sk-card-head">
                    <h3 class="sk-card-title">{{ __('profile.power_skills_section') }}</h3>
                    <p class="sk-card-copy">Rate the strengths you already use in real work situations.</p>
                </div>
                <div class="sk-hint">
                    <span class="material-symbols-outlined">info</span>
                    <span>{{ __('profile.power_skills_desc') }}</span>
                </div>
                <div class="sk-skill-grid">
                    ...
                    <div class="sk-skill-item">
                        <p class="sk-skill-name">{{ $skill->localized_name }}</p>
                        <p class="sk-skill-desc">{{ $skill->localized_description }}</p>
                        <span class="sk-pill-label">{{ __('profile.level') }}</span>
                        <input type="hidden" name="power_skills[{{ $skill->id }}]" value="0">
                        <div class="sk-pills">
                            <label class="sk-pill">
                                <input type="radio" name="power_skills[{{ $skill->id }}]" value="1" {{ $currentLevel == 1 ? 'checked' : '' }}>
                                <span>1 · {{ __('profile.level_basic') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="power_skills[{{ $skill->id }}]" value="2" {{ $currentLevel == 2 ? 'checked' : '' }}>
                                <span>2 · {{ __('profile.level_intermediate') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="power_skills[{{ $skill->id }}]" value="3" {{ $currentLevel == 3 ? 'checked' : '' }}>
                                <span>3 · {{ __('profile.level_advanced') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="power_skills[{{ $skill->id }}]" value="4" {{ $currentLevel == 4 ? 'checked' : '' }}>
                                <span>4 · {{ __('profile.level_expert') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="power_skills[{{ $skill->id }}]" value="5" {{ $currentLevel == 5 ? 'checked' : '' }}>
                                <span>5 · {{ __('profile.level_master') }}</span>
                            </label>
                        </div>
                    </div>
                    @endforeach
            ...
            <div class="sk-card">
                ...
                <div style="margin-bottom:28px;">
                    <div class="sk-category-chip">
                        ...
                        @if($category == 'cardinal')   {{ __('profile.cardinal_competencies') }}
                        @elseif($category == 'specific') {{ __('profile.specific_competencies') }}
                        @else {{ __('profile.technical_competencies') }}
                        @endif
                    </div>
                    <div class="sk-skill-grid">
                        ...
                        @endphp
                        <div class="sk-skill-item">
                            <p class="sk-skill-name">{{ $competency->localized_name }}</p>
                            <p class="sk-skill-desc">{{ $competency->localized_description }}</p>
                            <span class="sk-pill-label">{{ __('profile.level') }}</span>
                            <input type="hidden" name="behavioral_competencies[{{ $competency->id }}]" value="0">
                            <div class="sk-pills">
                                <label class="sk-pill">
                                    <input type="radio" name="behavioral_competencies[{{ $competency->id }}]" value="1" {{ $currentLevel == 1 ? 'checked' : '' }}>
                                    <span>1 · {{ __('profile.level_initial') }}</span>
                                </label>
                                <label class="sk-pill">
                                    <input type="radio" name="behavioral_competencies[{{ $competency->id }}]" value="2" {{ $currentLevel == 2 ? 'checked' : '' }}>
                                    <span>2 · {{ __('profile.level_developing') }}</span>
                                </label>
                                <label class="sk-pill">
                                    <input type="radio" name="behavioral_competencies[{{ $competency->id }}]" value="3" {{ $currentLevel == 3 ? 'checked' : '' }}>
                                    <span>3 · {{ __('profile.level_competent') }}</span>
                                </label>
                                <label class="sk-pill">
                                    <input type="radio" name="behavioral_competencies[{{ $competency->id }}]" value="4" {{ $currentLevel == 4 ? 'checked' : '' }}>
                                    <span>4 · {{ __('profile.level_advanced') }}</span>
                                </label>
                                <label class="sk-pill">
                                    <input type="radio" name="behavioral_competencies[{{ $competency->id }}]" value="5" {{ $currentLevel == 5 ? 'checked' : '' }}>
                                    <span>5 · {{ __('profile.level_expert') }}</span>
                                </label>
                            </div>
                        </div>
                        @endforeach
        ...
        <div class="sk-panel" id="sk-panel-3">
            <div class="sk-card">
                <div class="sk-card-head">
                    <h3 class="sk-card-title">{{ __('profile.organizational_culture') }}</h3>
                    <p class="sk-card-copy">Prioritize the environment and company values that matter most to you.</p>
                </div>
                <div class="sk-hint">
                    <span class="material-symbols-outlined">info</span>
                    <span>{{ __('profile.organizational_culture_advice') }}</span>
                </div>
                <div class="sk-skill-grid">
                    ...
                    @endphp
                    <div class="sk-skill-item">
                        <p class="sk-skill-name">{{ $value->localized_name }}</p>
                        <p class="sk-skill-desc">{{ $value->localized_description }}</p>
                        <span class="sk-pill-label">{{ __('profile.priority') }}</span>
                        <input type="hidden" name="culture_values[{{ $value->id }}]" value="0">
                        <div class="sk-pills">
                            <label class="sk-pill">
                                <input type="radio" name="culture_values[{{ $value->id }}]" value="1" {{ $currentPriority == 1 ? 'checked' : '' }}>
                                <span>1 · {{ __('profile.priority_critical') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="culture_values[{{ $value->id }}]" value="2" {{ $currentPriority == 2 ? 'checked' : '' }}>
                                <span>2 · {{ __('profile.priority_very_important') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="culture_values[{{ $value->id }}]" value="3" {{ $currentPriority == 3 ? 'checked' : '' }}>
                                <span>3 · {{ __('profile.priority_important') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="culture_values[{{ $value->id }}]" value="4" {{ $currentPriority == 4 ? 'checked' : '' }}>
                                <span>4 · {{ __('profile.priority_desirable') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="culture_values[{{ $value->id }}]" value="5" {{ $currentPriority == 5 ? 'checked' : '' }}>
                                <span>5 · {{ __('profile.priority_optional') }}</span>
                            </label>
                        </div>
                    </div>
                    @endforeach
        ...
        <div class="sk-panel" id="sk-panel-4">
            <div class="sk-card">
                ...
                <div class="sk-skill-grid">
                    ...
                    @endphp
                    <div class="sk-skill-item">
                        <p class="sk-skill-name">{{ $pref->localized_name }}</p>
                        <p class="sk-skill-desc">{{ $pref->localized_description }}</p>
                        <span class="sk-pill-label">{{ __('profile.importance') }}</span>
                        <input type="hidden" name="leadership_preferences[{{ $pref->id }}]" value="0">
                        <div class="sk-pills">
                            <label class="sk-pill">
                                <input type="radio" name="leadership_preferences[{{ $pref->id }}]" value="1" {{ $currentImportance == 1 ? 'checked' : '' }}>
                                <span>1 · {{ __('profile.importance_indispensable') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="leadership_preferences[{{ $pref->id }}]" value="2" {{ $currentImportance == 2 ? 'checked' : '' }}>
                                <span>2 · {{ __('profile.importance_very_important') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="leadership_preferences[{{ $pref->id }}]" value="3" {{ $currentImportance == 3 ? 'checked' : '' }}>
                                <span>3 · {{ __('profile.importance_important') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="leadership_preferences[{{ $pref->id }}]" value="4" {{ $currentImportance == 4 ? 'checked' : '' }}>
                                <span>4 · {{ __('profile.importance_desirable') }}</span>
                            </label>
                            <label class="sk-pill">
                                <input type="radio" name="leadership_preferences[{{ $pref->id }}]" value="5" {{ $currentImportance == 5 ? 'checked' : '' }}>
                                <span>5 · {{ __('profile.importance_valuable') }}</span>
                            </label>
                        </div>
                    </div>
                    @endforeach
